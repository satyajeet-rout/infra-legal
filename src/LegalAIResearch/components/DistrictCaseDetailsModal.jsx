...
const DistrictCaseDetailsModal = ({ caseDetails, isLoading, error, onClose }) => {
  ...
  const getAvailableTabs = () => {
    ...
    if (caseDetails.transfer && caseDetails.transfer.length > 0) tabs.push('transfer');
    if (caseDetails.firstInformationReport && Object.keys(caseDetails.firstInformationReport).length > 0) tabs.push('fir');
          ...
          {isLoading ? (
            ...
            <div>
              ...
              <div className="border-b mb-4">
                <div className="flex overflow-x-auto">
                  {getAvailableTabs().map((tab) => (
                    ...
                    >
                      {tab === 'fir' ? 'FIR' : tab.charAt(0).toUpperCase() + tab.slice(1)}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div className="mb-4">
                {activeTab === 'overview' && (
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {[
                        { label: 'Case Type', value: caseDetails.details?.type || 'N/A' },
                        { label: 'Filing Number', value: caseDetails.details?.filingNumber || 'N/A' },
                        { label: 'Filing Date', value: formatDate(caseDetails.details?.filingDate) },
                        { label: 'Registration Number', value: caseDetails.details?.registrationNumber || 'N/A' },
                        { label: 'Registration Date', value: formatDate(caseDetails.details?.registrationDate) }
                      ].map((item, index) => (
                        <div key={index}>
                      ...
                      {caseDetails.actsAndSections ? (
                        <div className="space-y-3">
                          <div className="border-b pb-3">
                            ...
                            <p className="text-sm font-medium">{caseDetails.actsAndSections.acts || 'N/A'}</p>
                          </div>
                          <div>
                            <p className="text-sm text-gray-500">Sections</p>
                            <p className="text-sm font-medium">{caseDetails.actsAndSections.sections || 'N/A'}</p>
                          </div>
                        </div>
                      ) : (
                    ...
                    {caseDetails.transfer?.length > 0 ? (
                      <div className="overflow-x-auto">
                        <table className="min-w-full border-collapse">
                          <thead>
                            <tr className="bg-gray-100">
                              <th className="border border-gray-300 p-2 text-left text-xs font-medium text-gray-500">Date</th>
                              <th className="border border-gray-300 p-2 text-left text-xs font-medium text-gray-500">From</th>
                              <th className="border border-gray-300 p-2 text-left text-xs font-medium text-gray-500">To</th>
                              <th className="border border-gray-300 p-2 text-left text-xs font-medium text-gray-500">Reason</th>
                            </tr>
                          </thead>
                          <tbody>
                            {caseDetails.transfer.map((item, index) => (
                              <tr key={index} className="hover:bg-gray-50">
                                <td className="border border-gray-300 p-2 text-sm">{formatDate(item.date)}</td>
                                <td className="border border-gray-300 p-2 text-sm">{item.from || 'N/A'}</td>
                                <td className="border border-gray-300 p-2 text-sm">{item.to || 'N/A'}</td>
                                <td className="border border-gray-300 p-2 text-sm">{item.reason || 'N/A'}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
                ...
                {activeTab === 'fir' && (
                  <div>
                    <h3 className="font-medium mb-4">First Information Report</h3>
                    {caseDetails.firstInformationReport && Object.keys(caseDetails.firstInformationReport).length > 0 ? (
                      <div className="bg-gray-50 p-4 rounded-md space-y-4">
                        {Object.entries(caseDetails.firstInformationReport).map(([key, value]) => (
